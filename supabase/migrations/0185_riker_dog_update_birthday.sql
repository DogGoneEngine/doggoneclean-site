-- 0185_riker_dog_update_birthday.sql
--
-- Bug: a Riker birthday update went into the void. Paul said "Gypsy birthday
-- 8/31/18, update her birthday in the records." Riker parsed it correctly into a
-- dog_update entry carrying birthday: "2018-08-31" (confirmed in riker_log), but
-- admin_riker_apply's dog_update branch only read price_cents and breed, so the
-- birthday was silently dropped on apply. The confirm summary said it would
-- update the birthday; nothing was written. (His first attempt landed it as a
-- dog note instead, which he did not want.)
--
-- Fix: the dog_update branch now also reads the birthday (accepting both
-- "birthday" and "birth_date" keys, since the model emits "birthday") and an
-- optional dob_approximate flag, and writes dogs.birth_date / dob_approximate.
-- No edge-function deploy is required: the model already emits the birthday;
-- only the apply side was throwing it away. Everything else is identical to the
-- live definition.
...
begin
  ...
  if v_client is not null then
    ...
    -- dog_update now also carries the birthday Riker parses ("birthday" or
    -- "birth_date" key) plus an optional dob_approximate flag.
