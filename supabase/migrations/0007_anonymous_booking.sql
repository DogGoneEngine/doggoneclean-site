-- 0007_anonymous_booking.sql
--
-- Match the proven Dog Gone Nails model: book WITHOUT an account. The
-- signup RPC no longer requires auth.uid(); it runs as anon (SECURITY
-- DEFINER), keyed on the phone number, and creates the subscriber +
-- dogs + subscription + first appointment. The auth_user_id stays NULL
-- until the client claims the portal later (phone OTP / Google match) in
-- a future slice. Reversing the earlier account-required choice (Paul,
-- 2026-05-29) after studying the nails flow: nails is the same business
-- (recurring, card-on-file, route-based) and requires no account to book,
-- so Clean should not either.
--
-- Adds the fields the nails flow captures that Clean was missing: a gate
-- code (The Villages gated communities), per-dog birth date + approximate
-- flag (already columns on bath_dogs), and the SMS opt-in.
--
-- Pricing is unchanged from 0005 (each dog at its own tier, stacking
-- per-additional-dog discount).

ALTER TABLE public.bath_subscribers
  ADD COLUMN IF NOT EXISTS gate_code text;

-- Replace the auth-required signature with the anonymous one.
DROP FUNCTION IF EXISTS public.bath_start_subscription(
  text, text, text, text, text, text, text, text, text,
  numeric, numeric, jsonb, text, timestamptz, text);
...
BEGIN
  ...
  -- Identity is the phone number. Reuse an existing subscriber row if one
  -- exists for this phone (returning client), else create a new anonymous
  -- one (auth_user_id stays NULL until the portal is claimed).
  ...
  IF v_subscriber IS NULL THEN
    INSERT INTO public.bath_subscribers (
      ...
      service_lat, service_lng, gate_code, city_id, sms_opt_in
    ) VALUES (
      p_first_name, p_last_name, p_email, p_phone_e164,
      p_address_line_1, p_address_city, p_address_state, p_address_zip,
      p_service_lat, p_service_lng, p_gate_code, v_city.id, COALESCE(p_sms_opt_in, true)
    )
