-- 0230_riker_notify_person_no_contact_graceful.sql
--
-- Bug: Paul told Clio "Notify Melody as well... for this week only" on the Amy
-- Blessing sheet, having given Melody's number a breath earlier as a who's-on-site
-- entry rather than in the same sentence. The parse carried a notify_person with
-- no phone and no email (confirmed in riker_log), and admin_upsert_notify_person
-- correctly raises 'phone or email required' on that. But that exception aborted
-- the WHOLE admin_riker_apply call, so the entire save errored out instead of
-- saving everything else and reporting the one problem.
--
-- Fix: the notify_person branch now checks for a phone or email BEFORE calling the
-- upsert RPC. With neither, it skips the notify person and sets a new result flag
-- notify_person_missing_contact = true, so the rest of the plan still applies and
-- the UI can tell Paul plainly that the notify person needs a number. Nothing else
-- changes; this is the live definition (which includes the equipment_service
-- branch) with only the notify_person branch guarded and one field added to the
-- returned object. No edge-function deploy is required.
