-- 0241_task_attachments_two_way.sql
--
-- Paul handed Jake a "find the appliance wattages" task and hit a wall twice:
-- Jake had no way to ENTER the answer back, and Paul had no way to ATTACH what
-- he already knew (a screenshot, a paste) when handing it off. And when Paul
-- spoke "the appliance wattages are ..." to Clio, she filed it to the wisdom
-- inbox because she could not see the open task that was asking for exactly
-- that. This migration makes a task a two-way channel and teaches Clio to match.
--
-- (1) task_attachments: an ordered, attributed list hung off a task. Either a
--     typed note or a file (screenshot/photo in the visit-photos bucket). Paul
--     attaches context when assigning; the assignee (Jake) attaches the answer
--     when finishing. RLS-on, no policies: every read/write rides the same
--     security-definer RPCs as the rest of the admin surface, never raw.
--
-- (2) admin_add_task_attachment: owner, the assignee, or the task's creator may
--     add a note or a file to a task. The file is uploaded to storage first;
--     this records its path. Honors the no-data-into-the-void rule: the answer
--     lands on the task itself, visible to whoever assigned it.
--
-- (3) admin_list_tasks: now returns each task's attachments inline, so Paul sees
--     what Jake entered and Jake sees what Paul attached, in the one panel.
--
-- (4) admin_riker_context: now also lists the OPEN tasks (id, title, details,
--     assignee), so Clio can match what Paul says to a task that is asking for
--     it instead of dropping it in the wisdom inbox.
--
-- (5) admin_riker_apply: a plan can now carry task_attachment {task_id, note}.
--     When present and the task exists, the note lands as an attachment on that
--     task (author = Paul). Falls through to wisdom when no task matches.
--
...
-- Add a note or a file to a task. Owner, the assignee, or the creator only.
create or replace function public.admin_add_task_attachment(
  p_task_id uuid, p_kind text, p_body text default null, p_storage_path text default null)
returns uuid language plpgsql security definer set search_path to ''
as $$
declare v_me uuid; v_role text; t public.tasks%rowtype; v_name text; v_id uuid;
begin
  ...
  if v_role <> 'owner' and t.assigned_to <> v_me and t.created_by <> v_me then
    raise exception 'not your task';
  end if;
  if p_kind not in ('note', 'file') then raise exception 'bad attachment kind'; end if;
  if p_kind = 'note' and nullif(btrim(coalesce(p_body, '')), '') is null then
    raise exception 'note text required';
  end if;
  if p_kind = 'file' and nullif(btrim(coalesce(p_storage_path, '')), '') is null then
  ...
  insert into public.task_attachments (task_id, author_id, author_name, author_role, kind, body, storage_path)
  values (p_task_id, v_me, nullif(v_name, ''), v_role, p_kind,
          nullif(btrim(coalesce(p_body, '')), ''), nullif(btrim(coalesce(p_storage_path, '')), ''))
  returning id into v_id;
  return v_id;
end;
$$;
revoke all on function public.admin_add_task_attachment(uuid, text, text, text) from public, anon;
grant execute on function public.admin_add_task_attachment(uuid, text, text, text) to authenticated, service_role;

-- admin_list_tasks: identical to the live definition, plus an 'attachments'
-- array per task (notes and files, oldest first) so the two-way thread shows.
  ...
  return coalesce((
    select jsonb_agg(jsonb_build_object(
      ...
      'attachments', coalesce((
        select jsonb_agg(jsonb_build_object(
          'id', x.id, 'kind', x.kind, 'body', x.body, 'storage_path', x.storage_path,
          'author_name', x.author_name, 'author_role', x.author_role, 'created_at', x.created_at)
          order by x.created_at)
        from public.task_attachments x where x.task_id = t.id), '[]'::jsonb)
...
-- admin_riker_context: identical to the live definition, plus 'open_tasks' in
-- both branches so Clio can match a spoken answer to the task that asked for it.
...
-- admin_riker_apply: identical to the live definition, plus task_attachment
-- handling. A plan carrying task_attachment {task_id, note} lands the note on
-- that open task as an attachment (author = the caller, Paul). Added to the
-- "is there anything to do" guard so a pure task-answer plan is valid.
...
begin
  ...
  -- Task answer: Paul speaks the information an open task is asking for; it
  -- lands as a note attachment on that task instead of the wisdom inbox.
