-- 0231_riker_access_note_and_charge_tip.sql
--
-- Two field-routing fixes found at the Emily Cummings appointment (2026-06-22):
--
-- 1. Getting-in / arrival instructions now have a real home. Riker emits a new
--    top-level access_note (a gate code, "do not knock", "text on arrival"), and
--    this apply writes it to clients.access_notes, which is the "Getting in" line
--    the Today appointment card actually shows. Before, such instructions landed
--    in the household note, which never appears at the door.
-- 2. "Charged X, paid Y" is recorded in full. The visit object now carries
--    charged_cents and tip_cents alongside amount_cents (the amount collected),
--    so charged -> charged_cents, paid -> amount_collected_cents, and the tip ->
--    tip_cents instead of everything collapsing to one number.
--
-- Rebuilt from the live definition (which already had the equipment_service and
-- notify-person-skip branches); only the access_note branch, the two new visit
-- columns, and two return fields are added. The edge-function prompt + schema are
-- updated in the same commit; redeploy the riker function so the model emits the
-- new fields.
...
begin
  ...
  -- Getting-in / arrival instruction -> clients.access_notes (the "Getting in"
  -- line shown on the appointment). Appends so a gate code and a knock rule can
  -- coexist; Paul edits on the sheet if one supersedes another.
  ...
  if v_client is not null and jsonb_typeof(p_plan->'visit') = 'object' then
    ...
    if v_visit is not null then
      ...
      insert into public.visits (
        ...
        work_done, visit_notes, actual_minutes, amount_collected_cents, charged_cents, tip_cents, payment_method, source, completed_by
      ) values (
  ...
  if v_client is not null then
    ...
    if jsonb_typeof(v_np) = 'object' then
      -- A notify person with neither phone nor email cannot be messaged, and the
      -- upsert RPC raises on that. Skip it and report back rather than abort the
      -- whole plan, so one contactless name never throws away every other change.
