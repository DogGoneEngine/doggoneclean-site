-- 0134: excluded_breeds_are_slide_holes (Oracle, Paul 2026-06-10).
-- The hard breed exclusions for new v2 signups get database teeth:
-- bath_start_subscription rejects an excluded breed before any row is
-- written, so no booking path (and no future funnel redesign) can book a
-- doodle, poodle cross, Siberian Husky, Great Pyrenees, or Great Dane.
-- The funnel shows a kind early decline; this is the gate behind it.
-- Legacy clients are untouched (they live in clients/dogs and are
-- grandfathered; this guard only runs on the public signup RPC).
...
BEGIN
  ...
  LOOP
    -- excluded_breeds_are_slide_holes: the hard exclusions, declined kindly
    -- before any row is written. Coats and sizes that need haircut-level
    -- work or hours a route stop cannot give.
    IF (d->>'breed') ~* '(doodle|poodle|husky|huskies|pyrenees|great\s*dane)' THEN
      RAISE EXCEPTION 'We are honestly not built for % dogs: doodles and poodle crosses, Siberian Huskies, Great Pyrenees, and Great Danes need more time than a mobile route stop allows. A full-service dog grooming salon is the right home for that coat.',
