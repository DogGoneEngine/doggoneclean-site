-- 0009_require_verified_service_area.sql
--
-- Close the manual / unverified-address path at the server.
--
-- 0008 made the in-polygon check authoritative but still ACCEPTED a signup
-- with no coordinates (manual entry / dead autocomplete) as
-- address_verified = false, punting verification to "the operator confirms
-- before the first visit." Paul's rule is that there is NO manual path: an
-- address either autocompletes and falls inside the route, or it cannot book.
-- The browser manual-entry fallback is removed (BookingApp.jsx); this makes
-- the server agree, so a crafted coordinate-less request cannot create an
-- unverified row either.
--
-- Behavior of bath_start_subscription after this migration:
--   - coordinates absent            -> REJECT (hard error). No row written.
--   - coordinates present, outside  -> REJECT (hard error). No row written.
--   - coordinates present, inside   -> proceed; address_verified = true.
-- The address_verified column stays (always true on a successful booking now)
-- so the future Stripe charge job retains its belt-and-suspenders guard.
...
BEGIN
  ...
  -- Service-area gate (authoritative, server-side). No manual / unverifiable
  -- path: a booking MUST carry coordinates that fall inside the city polygon.
  -- Missing coordinates (autocomplete bypassed, crafted request) and an
  -- out-of-area point are both hard-rejected here, before any row is written.
  -- The browser autocomplete is convenience; THIS is the line nothing walks past.
