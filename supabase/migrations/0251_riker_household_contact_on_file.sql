-- 0251_riker_household_contact_on_file.sql
--
-- Teach Clio (the riker capture) to handle "add a phone number for <household
-- member>" instead of dropping it. Three gaps caused the Tonya Hunt / "Bo" miss:
--   1. With a client sheet open, admin_riker_context returned the household's
--      notify_people but NOT its aliases, so the household name "Bo Hunt" was
--      invisible to the parser and it answered "there is no person named Bo".
--   2. admin_riker_apply had no way to carry an active flag, so a person added by
--      phone always landed active (would get messages). Paul wants a household
--      member's number ON FILE with notifications OFF, toggleable when needed.
-- This migration fixes the two SQL pieces; the riker edge function prompt is
-- updated alongside to route a non-client person's phone/email into notify_person
-- and to set active only when Paul says they should actually be messaged.

-- 1) Context: surface the household's aliases when a sheet is open, so the parser
--    can resolve a household name (e.g. "Bo" -> the listed "Bo Hunt").
...
-- 2) Apply: after upserting a notify person, honor an explicit active flag from
--    the plan; a brand-new person (no prior id) defaults to NOT notified, so a
--    household member's number lands on file with the toggle off.
  ...
  if v_client is not null then
    ...
    if jsonb_typeof(v_np) = 'object' then
      if nullif(btrim(coalesce(v_np->>'phone','')),'') is null
        ...
        -- Honor the notify toggle: an explicit active in the plan always wins
        -- (turning a known person on or off comes with their id). A brand-new
        -- person added without an explicit active defaults to OFF, so a household
        -- member's number sits on file until Paul toggles it on.
        if v_np_id is not null then
          if nullif(v_np->>'active','') is not null then
            update public.notify_people set active = (v_np->>'active')::boolean where id = v_np_id;
          elsif nullif(v_np->>'id','') is null then
            update public.notify_people set active = false where id = v_np_id;
          end if;
        end if;
      end if;
    end if;
