...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const system = [
      ...
      "A standing instruction (not about getting in), or anything about how to do the work next time, goes in client_note (household-level) or dog_notes (about one dog). Behavior or condition seen this visit goes in visit.visit_notes.",
      ...
      "ONE person goes in ONE place. Pick the single field that matches what Paul asked: alias_add for a household name, onsite_update for who is at the house, notify_person for who gets the appointment messages. Do NOT also copy that person into the other fields. 'Add only X' means add X exactly once. Use more than one field for the same person ONLY when Paul explicitly asks for more than one of those things.",
