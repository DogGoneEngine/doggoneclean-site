# Hurricane Bath - website showcase (drop-in content)

**What this is.** The finished content for the Hurricane Bath showcase, the hero of Clean's
bath-forward marketing. It is written to drop into the Astro marketing site when that is
scaffolded. No component, page, or animation is built yet, on purpose (per `elons_algorithm`,
the build waits for the site; what is scarce now is the content, not the code).

**Why it matters.** The Hurricane Bath is Clean's un-promptable differentiator, so this is a
`dig_the_moat` asset: it sells what the bath does and that Paul invented it, while keeping the
build private.

**Status: DRAFT.** Grounded in Paul's own account (2026-05-24). The shampoo brand and the build
specs stay in the private record. Nothing here is invented (`real_data_only`), and per
`dont_knock_competitors` it never names or knocks other systems.

---

## The moat decision (read first)

**Sell the what, protect the how.** The only truly copyable part is the parts list (the exact
pumps, their power, the plumbing). Everything else (the result, Paul's judgment, years of
refinement, the cleaning discipline) cannot be copied off a web page, which is why it is the moat.
Be generous with the benefits and the story; keep the build recipe private (CLEAN_FIELD_MANUAL.md,
"The Hurricane Bath").

---

## Public showcase (the drop-in content)

### Hero
Headline (recommended): **Built, not bought.**
Subhead: "The Hurricane Bath moves up to about ten gallons a minute, the kind of flow you get from
city water, and dials all the way down to a soft trickle for a dog's face and eyes. Paul built it
himself, and it gets a dog genuinely clean, fast."

### Why he built it
None of the systems Paul could buy did everything he wanted in one machine. So he built the one
that does. Years of working with the professional systems showed him what to keep and what was
missing, and he put all of it into a single machine tuned exactly the way he wants to wash a dog.
There is nothing else quite like it, and he is still refining it.

### Power, dialed to the dog
A lever right at the nozzle sets the water anywhere from a soft trickle to full force. Full force
drives water through a thick double coat all the way to the skin, which is where a real clean
happens. A soft setting is right for a face, eyes, and sensitive spots. No fire-hosing where it
does not belong. Powerful does not have to mean rough.

### Your dog finishes on clean water
The wash recirculates, and it gets a dog clean the same way a dishwasher does: the water moves
through the system over and over, and what comes out is clean. Every dog finishes the same way,
with a fresh-water rinse from a separate tank.

### No dog too dirty
A dog caked head to toe in mud is not a problem. When the wash water turns to a mud puddle, Paul
flushes the whole system, refills it with clean water, and washes again, as many times as it
takes, until that dog is washing up like an already-clean dog. There is very little a coat can
hold that the Hurricane Bath cannot get out.

### Shampoo that suits your dog
Clean washes every dog with a gentle shampoo and conditioner that sits well with nearly every
coat and skin. If your dog needs something specific, a medicated or prescription shampoo or
anything for a particular condition, bring it and we will use it. The bottle loads into the
system before the bath even starts, so it is back in your hands before your dog is wet.

### Easy on water
Most dogs are washed and rinsed with under five gallons. The power comes from how the system moves
water, not from wasting it.

### Fast
The system is quick. A faster bath is a calmer, shorter visit for your dog, and that speed is what
makes washing a very dirty dog twice over no trouble at all.

### The experience
Powerful does not mean rough. The pressure dials down to a soft trickle for a face or a nervous
dog, the trailer stays the calm, familiar place, and the dog comes out genuinely clean.

### The name (optional, light touch)
The name leans into Florida: a hurricane washes everything clean. Use only if it adds something;
no big origin story needed.

### Proof and credibility
Assets Paul will provide (data gaps):
- Video: the dial-down-then-up technique driving water to the skin on a double coat, and a
  flush-and-rewash going from muddy water to clean.
- A short clip or photo set of the system, curated so it does not hand over the build.
- Before-and-after dogs, including a genuinely filthy one.
- A number if you want one (years in service, dogs bathed).

### FAQ (draft)
- **Is it gentle?** Yes. The pressure dials down to a soft trickle, so faces, eyes, and small or
  nervous dogs get a soft bath.
- **Is my dog washed in clean water?** The wash recirculates and gets your dog clean the way a
  dishwasher does, an extra-dirty dog is flushed and washed again with fresh water, and every dog
  finishes on a clean-water rinse from a separate tank.
- **What shampoo do you use?** A gentle shampoo and conditioner that suits nearly every dog. If
  your dog needs something specific, medicated, or for a particular condition, bring it and we
  will use it.
- **How dirty is too dirty?** Just about nothing. Mud from nose to tail is a normal day.

---

## Open items
- **Assets:** photos, video, before-and-afters (and which are safe to show without revealing the
  build).
- **Confirm:** the "Built, not bought" headline and the overall voice.
- **Resolved:** the house shampoo brand stays unnamed on the page; the coyote story stays off the
  public page (kept internal); the flea detail is trimmed to a simple "bring what your dog needs."
- **For intake/terms, not the marketing page:** any wording that Clean does not guarantee a
  client-supplied medicated or flea product will work belongs in the intake or terms fine print,
  where a disclaimer is appropriate, never in the hero copy (`reminder_voice`, `house_shampoo`).

## Language bank (gold lines, flagged for future use)

Strong lines pulled from the internal coyote story; work them into public copy when they fit
(`language_bank`).

- "A garden hose would have laughed at that dog. A bath by hand would have been an all-day act of
  surrender." Safe and strong: it pokes at backyard methods, not at any product or competitor.
- "Most setups would have just smeared it around." Hold or adapt. This one edges into knocking
  other systems (`dont_knock_competitors`). Keep the rhythm, drop the knock, for example fold the
  punch into the garden-hose line or end on "and the mud did not stand a chance."
- "By the end, the only thing left on him was the smell of a job well done." Gold, safe, ready.
- "...a coat you would never guess had been a crime scene an hour earlier." Adapt the "crime
  scene" for general use, for example "a coat you would never guess had been a disaster an hour
  earlier" or "...had been anything but spotless."

## Private record (DO NOT PUBLISH)

The build is proprietary and is the moat. The canonical record lives in CLEAN_FIELD_MANUAL.md
("The Hurricane Bath"), and the shampoo brand and policy in "Shampoo and product policy." This
showcase references the outcome, never the schematic or the brand.

### Internal story (DO NOT PUBLISH): The Mud-Caked Guardian

Kept for internal use and as a capability story, not for the public page.

Somewhere out past the fence line, a livestock guard dog was doing the job he was bred for. The
foals were his to watch, all wobbly legs and no sense, and a coyote had made the mistake of its
short life by deciding they looked like dinner.

It did not go the coyote's way.

By the time the dust settled, the guardian had run the thing down through a low, wet, churned-up
stretch of ground and finished the fight. He came trotting back the way heroes do, head high,
tail easy, completely unbothered, and absolutely unrecognizable. Mud to the eyebrows. Mud in
places mud has no business being. The whole story of the morning was written across his coat in
earth and worse, packed down deep into a thick double coat that does not give anything up easily.

A garden hose would have laughed at that dog. A bath by hand would have been an all-day act of
surrender. Most setups would have just smeared it around.

So Paul loaded him into the trailer and went to work. Dial the pressure down, soak the worst of
it loose. Flush the system when the water turned to a churning brown soup, refill, go again. The
mud puddle came out, clean water went in. Wash like he is filthy, then wash him like he is dirty,
then wash him like he is just a regular dog having a regular bath. By the end, the only thing
left on him was the smell of a job well done and a coat you would never guess had been a crime
scene an hour earlier.

The foals lived. The coyote did not. And the hero got the one bath in the county that could
actually keep up with him.
