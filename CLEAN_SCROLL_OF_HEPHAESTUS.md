...
- Garret Little: nails-only, ~20 quick visits in time_is_money, but NO dog names are recorded on any findable sheet, so his dogs cannot be added without inventing them. Amanda Batson was merged into his household; her dogs are likewise unrecorded. Left as a dog-data gap (same posture as Steve Crandall and Nancy Franklin's nail-only gaps). If Paul can name Garret's dog(s), add them and they get a record.
...
Carry-forward cleanup still open: Kevin's and Mary Jane's pre-2024 tails; fold Cynthia's guest Stella and Mary Beth's guest Benji into visit_notes.
