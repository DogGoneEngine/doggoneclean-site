...
### 2026-06-09 (time_is_money export shrinks to a small link; by-hand "missed stop" entry added)

The export had been a full panel/button near the top of the Clients floor and Paul kept forgetting where it lived. Shrank it to a tiny underlined "time is money" link, right-aligned and out of the way, that opens the whole thing. Inside that one place now lives both the export (unchanged) and a collapsed "Add a stop by hand (forgot to tap)" form for the case we discussed: when Paul forgets to tap the Left/Arrived/Done buttons on the Today sheet. The by-hand form takes client + date + the three times + Charged/Paid/Method and writes a source='manual' visit via the existing admin_log_visit, so it flows into the same admin_export_time_is_money block (which reads source in manual/appointment, joins the client name, renders times in America/New_York). No new RPC needed; it reuses the visit spine. The manual form stays collapsed so it never takes room on the main screen, matching Paul's ask that the option exist where the data lives but not on the front screen. UI in ClientsView.jsx (TimeIsMoneyExportPanel + ManualStopForm).
