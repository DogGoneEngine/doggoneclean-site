...
| service_area_enforced_server_side | Engineering | Oracle; DB `_bath_point_in_area` + `bath_start_subscription` hard-rejects coordinate-less and out-of-area (migration 0009); `BookingApp.jsx` autocomplete-only gate, no manual path; **`check.py`** bans manual-entry copy in the booking island AND fails the build if a polygon ring is hard-coded under `src/` (polygon must come from `cities.polygon`) | charge job still honors `address_verified` as a guard |
...
| villages_only_at_launch | Hurricane Bath: product | Oracle; DB `bath_start_subscription` rejects coordinate-less and out-of-area via `_bath_point_in_area` (authoritative, migration 0009); client `maps.js` `isInServiceArea` as UX | (none) |
...
| three_dog_cap | Hurricane Bath: product | Oracle; DB `bath_appointments.dog_count` CHECK (1-3) + RPC; **`check.py`** FORBIDS a customer-facing "three dogs" statement (the cap is the Villages HOA limit, not a Dog Gone rule, so it is never stated to customers as ours) | booking flow dog-count limit |
