...
| pizza_tracker_client_loop | Hurricane Bath: ux | Oracle; spec + build order in CLEAN_PARKING_LOT.md; `bath_appointments.status` already carries the progress states; `visit_photos` already stores per-visit photos | tracker page + token fetch; on-my-way send (Twilio-gated); review_asks tracking table; tip ask (Stripe-gated) |
