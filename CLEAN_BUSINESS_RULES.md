...
| rpc_grants_explicit | Engineering | Oracle; DB grants per tier (migration 0135) + `ALTER DEFAULT PRIVILEGES ... REVOKE EXECUTE ... FROM PUBLIC` so new functions are born locked; verified: anon denied on admin RPCs, anon booking RPCs intact, authenticated keeps the gated admin path | a check.py lint that scans new migrations for CREATE FUNCTION without an explicit GRANT |
...
| excluded_breeds_are_slide_holes | Hurricane Bath: product | Oracle; DB `bath_start_subscription` rejects excluded breeds before any row is written (migration; the durable teeth); funnel shows a kind early decline per dog; **`check.py`** asserts "doodle" present on the booking island | breeds.json eligibility data when the questionnaire lands |
...
| pizza_tracker_client_loop | Hurricane Bath: ux | Oracle (client name: the Dog Gone Tracker); DB `bath_appointments.tracker_token` + anon `tracker_status` RPC + `admin_on_my_way` (migration 0136, explicit grants); `/track?t=` page (stage timeline off status + the time stamps); Today-sheet "On my way" button (flips status, stamps Left, shares the heads-up message); `review_asks` table; photo sharing (0137): per-photo Orbit Share toggle (admin_set_photo_visibility), portal Photos section (bath_my_visit_photos + client-scoped storage policy), operator name + photo on the tracker page; `tracker-photos` edge fn (token-scoped, server-signed URLs) + the /track photo strip | automated send (Twilio-gated); review-ask send + click tracking; tip ask (Stripe-gated) |
...
| stop_sign_two_taps | Hurricane Bath: ux | Oracle; **`check.py`** asserts "two taps" on `index.astro`, `the-villages.astro`, `book.astro`, `terms.astro`, `PortalApp.jsx`; LIVE portal flow: red stop-sign control in Account > Your plan (PlanActions `.pt-stop`), confirm screen with the slot-release line, two taps total, `bath_cancel_subscription` RPC | n/a |
