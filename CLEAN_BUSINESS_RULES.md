...
| dog_standing_instructions | Clean: clients | Oracle; `dogs.standing_instructions` (0081), distinct from `dogs.notes` / `visits.visit_notes`; `admin_set_dog_standing`; shown + editable per dog on the contact sheet (DogCard) | populate from Drive contact sheets (cross-reference pass); let Riker write it |
